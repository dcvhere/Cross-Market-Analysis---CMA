...
if selected == "Home":
    ...
    if crypto_option == "Current Market Data":
        ...
        try:
            df_crypto_current = pd.read_sql("SELECT * FROM crypto_current_market LIMIT 100", engine)
        ...
        try:
            df_top_5_crypto = pd.read_sql("SELECT name, symbol, current_price, market_cap FROM crypto_processed_current ORDER BY market_cap DESC LIMIT 5", engine)
        ...
        try:
            df_high_supply_ratio = pd.read_sql("SELECT name, symbol, circulating_supply, total_supply FROM crypto_processed_current WHERE (circulating_supply * 100 / total_supply) > 90 LIMIT 10", engine)
    ...
    if oil_option == "Raw Oil Prices":
        ...
        try:
            df_oil_raw = pd.read_sql("SELECT date, price FROM oil_prices ORDER BY date DESC LIMIT 100", engine)
        ...
        try:
            df_highest_oil = pd.read_sql("SELECT MAX(price) AS highest_oil_price_5y FROM oil_processed_prices WHERE date >= DATE_SUB(CURDATE(), INTERVAL 5 YEAR)", engine)
        ...
        try:
            df_avg_annual_oil = pd.read_sql("SELECT YEAR(date) AS year, AVG(price) AS average_price FROM oil_processed_prices GROUP BY year ORDER BY year DESC LIMIT 5", engine)
    ...
    if stock_option == "Recent Stock Prices":
        ...
        try:
            df_stocks_raw = pd.read_sql("SELECT date, source, close_price FROM stock_prices ORDER BY date DESC LIMIT 100", engine)
        ...
        try:
            df_highest_nasdaq = pd.read_sql("SELECT MAX(close_price) AS highest_close_price_NASDAQ FROM stock_processed_data WHERE source = '^IXIC'", engine)
        ...
        try:
            df_sp500_diff = pd.read_sql("SELECT date, (high_price - low_price) AS price_difference FROM stock_processed_data WHERE source = '^GSPC' ORDER BY price_difference DESC LIMIT 5", engine)
